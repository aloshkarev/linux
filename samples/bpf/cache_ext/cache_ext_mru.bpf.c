...
{
	dbg_printk("cache_ext: Hi from the mru_init hook!\n");
...
{
	dbg_printk("cache_ext: Hi from the mru_folio_added hook!\n");
	if (!is_folio_relevant(folio))
		return;

	if (bpf_cache_ext_list_add(mru_list, folio))
...
{
	dbg_printk("cache_ext: Hi from the mru_folio_accessed hook!\n");

	if (!is_folio_relevant(folio))
		return;

	if (bpf_cache_ext_list_move(mru_list, folio, false))
...
{
	dbg_printk("cache_ext: Hi from the mru_folio_evicted hook!\n");
