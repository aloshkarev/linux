...
static int validate_watch_dir(const char *watch_dir, char *watch_dir_full_path)
{
	...
	if (!realpath(watch_dir, watch_dir_full_path)) {
		perror("realpath");
		return 1;
	}
...
int main(int argc, char **argv)
{
	...
	if (initialize_watch_dir_map(watch_dir_path,
				     bpf_map__fd(inode_watchlist_map(skel)),
				     false)) {
	...
	events = ring_buffer__new(bpf_map__fd(skel->maps.events),
				  handle_event, &reconfigure_prog_fd, NULL);
	if (!events) {
	...
	printf("cache_ext LHD policy attached to %s; press Ctrl-C to detach\n",
	       args.cgroup_path);
	while (!exiting) {
		ret = ring_buffer__poll(events, -1);
		if (ret == -EINTR) {
			ret = 0;
			break;
		} else if (ret < 0) {
